DDO Layout Editor
A browser-based editor for Dungeons & Dragons Online `.layout` files.
...
Live: https://longfreename.github.io/ddo-layout-editor/
...
---
Getting started
Export from the game. In DDO's chat box type `/ui layout save myui.layout`. The game reports where it saved it — normally `Documents\Dungeons and Dragons Online\UI\layouts`.
Open it here. Use Open .layout, or drag the file anywhere onto the page.
Edit, then save back into that same folder.
Reload in game:
```
   /alias clear
